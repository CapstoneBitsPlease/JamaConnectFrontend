...
const SelectItem = () => {
  ...
  const checklinked = useStoreActions(
    (actions) => actions.jamaitem.checklinkingpage
  );
  ...
  const get_prog = () => {
    axios
      .get("http://127.0.0.1:5000/jama/projects", {
        headers: {
  ...
  const get_type = () => {
    axios
      .get("http://127.0.0.1:5000/jama/item_types", {
        headers: {
          "Access-Control-Allow-Origin": "*",
          "Access-Control-Allow-Method": "GET,PUT,POST,DELETE,OPTIONS",
          Authorization: `Bearer ${token}`,
  ...
  const get_list = () => {
    axios
      .get(
        `http://127.0.0.1:5000/jama/items_by_type?type_id=${types_id}&project_id=${projects_id}`,
        {
          headers: {
            "Access-Control-Allow-Origin": "*",
            "Access-Control-Allow-Method": "GET,PUT,POST,DELETE,OPTIONS",
  ...
  const check_error = () => {
    axios
      .get(`http://127.0.0.1:5000/jama/item_by_id?item_id=${item_id}`, {
        headers: {
          "Access-Control-Allow-Origin": "*",
          "Access-Control-Allow-Method": "GET,PUT,POST,DELETE,OPTIONS",
          Authorization: `Bearer ${token}`,
  ...
  const check_error2 = () => {
    axios
      .get(`http://127.0.0.1:5000/jira/item_by_id?id=${jira_id}`, {
        headers: {
          "access-control-allow-origin": "*",
          "access-control-allow-method": "get,put,post,delete,options",
  ...
  const check_again = () => {
    ...
    if (testjamaid && testjiraid && testtoken) {
      makeToast(
        "success",
        "Both IDs are valided, sucessfully redirect to link field"
      );
      checklinked(true);
    } else if (!testjamaid) {
      settestjamaid(false);
  ...
  return (
    <div className="select_item-container">
      <form className="select_item-selecting">
        <h1 className="select_item-title">Item Selection</h1>
